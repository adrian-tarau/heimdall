...
public class GithubProviderFactory implements Provider.Factory {
    ...
    private void registerModels(net.microfalx.heimdall.llm.api.Provider.Builder builder) {
        builder.model(Model.create("gpt_4o", "GPT 4o")).tag("openai").tag("gpt");
        builder.model(Model.create("gpt_4.1", "GPT 4.1")).tag("openai").tag("gpt");
        builder.model(Model.create("o1", "O1")).tag("openai");
        builder.model(Model.create("o3", "O3")).tag("openai");
        builder.model(Model.create("o3_mini", "O3 Mini")).tag("openai").tag("mini");
        builder.model(Model.create("o4_mini", "O4 Mini")).tag("openai").tag("mini");
