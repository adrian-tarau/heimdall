...
public class OllamaProviderFactory implements Provider.Factory{
    ...
    private void registerModels(net.microfalx.heimdall.llm.api.Provider.Builder builder) {
        builder.model(Model.create("gemma3:4b","Gemma3 (4b)")).tag("meta").tag("gemma");
        builder.model(Model.create("qwen3:0.6b","Qwen3 (0.6b)")).tag("alibaba").tag("qwen3");
        builder.model(Model.create("deepseek-r1:7b","DeepSeek-R1 (7b)")).tag("deepseek").tag("deepseek-r1");
    }
}
