...
public class OpenAiProviderFactory implements Provider.Factory {
    ...
    private void registerModels(net.microfalx.heimdall.llm.api.Provider.Builder builder) {
        builder.model(Model.create("o4-mini", "O4 Mini")).tag("openai").tag("mini");
        builder.model(Model.create("o3", "O3")).tag("openai").tag("o3");
        builder.model(Model.create("o3-mini", "O3 Mini")).tag("openai").tag("mini");
        builder.model(Model.create("o1", "O1")).tag("openai").tag("o1");
        builder.model(Model.create("gpt_4.1", "GPT 4.1")).tag("openai").tag("gpt");
        builder.model(Model.create("gpt-4o", "GPT 4o")).tag("openai").tag("gpt");
        builder.model(Model.create("gpt_4.1_mini", "GPT 4.1 Mini")).tag("openai").tag("gpt").tag("mini");
        builder.model(Model.create("gpt_4o_mini", "GPT 4o Mini")).tag("openai").tag("gpt").tag("mini");
        builder.model(Model.create("gpt_4.1_nano", "GPT 4.1 Nano")).tag("openai").tag("gpt").tag("nano");
    }
}
